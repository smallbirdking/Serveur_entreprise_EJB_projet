...
public class PetSellerTest {
    ...
    public static Archive<?> createDeployment() {
        return ShrinkWrap.create(WebArchive.class, "test.war").addPackage(
                PetSeller.class.getPackage()).addAsWebInfResource(
                EmptyAsset.INSTANCE, "beans.xml").addAsWebInfResource(
    ...
    public void testRetrieveCats() throws Exception {
        ...
        try {
            ...
            CriteriaQuery<PetCard> criteria =
                    builder.createQuery(PetCard.class);
            Root<PetCard> from = criteria.from(PetCard.class);
            criteria.select(from);
            criteria.where(builder.like(from.<Pet> get("pet").<String> get(
                    "name"), "cat%"));
            ...
            for (int i = 0; i < result.size(); i++) {
                if (i > 0) {
                    Assert.assertTrue(last.price.compareTo(result.get(i).price) >= 0);
                }

                Assert.assertTrue(result.get(i).pet.name.startsWith("cat"));
